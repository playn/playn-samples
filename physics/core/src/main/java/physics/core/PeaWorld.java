...
import physics.core.entities.*;

public class PeaWorld implements ContactListener {

  public final Physics game;
  ...
  public PeaWorld (Physics game, GroupLayer scaledLayer) {
    this.game = game;
  ...
  public RFuture<PeaWorld> loadLevel (final Platform plat, String levelName) {
    return game.plat.assets().getText(levelName).flatMap(new Function<String,RFuture<PeaWorld>>() {
      public RFuture<PeaWorld> apply (String levelJson) {
        return loadLevel(game.plat, game.plat.json().parse(levelJson));
      }
    });
  }
  ...
  public Image getEntityImage (String name) {
    ...
    if (image == null) entityImages.put(path, image = game.plat.assets().getImage(path));
    return image;
  }
